...
const Filters = () => {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <button
        className="bg-black text-white font-bold rounded py-2 px-4 items-center w-[100%] m"
        onClick={() => {
          setYear(YEARS[0]);
          setSort(FILTERS[0]);
        }}
      ...
      <select
        className="w-[100%] text-lg p-2 pe-4 rounded-xl"
        onChange={(e) =>
          setSort(
            FILTERS?.find(
      ...
      <select
        ...
        onChange={(e) =>
          setYear(
            YEARS?.find((item) => item === Number(e.target.value)) || YEARS[0]
          )
        }
      >
        {YEARS.map((item) => (
